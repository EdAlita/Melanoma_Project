...
class MFR:
    ...
    def _filter_kernel_mf_fdog(self, t = 3, mf = True):
        ...
        if two_sigma_sq == 0:
            two_sigma_sq = 1
        div = np.sqrt(2 * np.pi) * self.sigma
        if div == 0:
            div = 1.
    ...
    def fdog_filter_kernel(self, t = 3):
        '''
        K = - (x/(sqrt(2 * pi) * sigma ^3)) * np.exp(-x^2/2sigma^2), |y| <= L/2, |x| < s * t
        '''
        return self._filter_kernel_mf_fdog(t, False)

    def gaussian_matched_filter_kernel(self, t = 3):
        '''
        K =  1/(sqrt(2 * pi) * sigma ) * np.exp(-x^2/2sigma^2), |y| <= L/2, |x| < s * t
        '''
...
def pipeline(path, params = params, filter_out = True, save = True):
    ...
    bank_gf = matched.createMatchedFilterBank(gf, rot)
    bank_fdog = matched.createMatchedFilterBank(fdog, rot)
    ...
    # get rid of the segment less than n pixel 
    if filter_out:
        lab = 1
        ...
        for y in range(h):
            for x in range(w):
                if num[int(label[y][x]-1)] <= n:
                    out[y][x] = 0

    # generate the output images. 
    if save: 
        cv2.imwrite("Final_gray_n10.jpg", out)
        cv2.imwrite("MF_gray_n10.jpg", H)
